...
block = web3.eth.getBlock(block_num_once)
print("-----------------------------------")
print("Hash block use block.hash : ",hash)
print("-----------------------------------")
list_trasaction_json = json.loads(web3.toJSON(list_trasaction))
print("Transaction in block JSON: ",list_trasaction_json)
for transaction in list_trasaction_json:
    data["transaction_id"].append(transaction)
print("-----------------------------------")
transaction_in_block = json.dumps(data)
